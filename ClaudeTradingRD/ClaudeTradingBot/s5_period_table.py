"""Every snap-model variant evaluated over the REAL account's exact window
(2026-06-21 22:00 -> 06-24 00:05). Maker spread 0.20, $10/pt (0.10 lot).
NOTE: this is a ~2-day, ~578-bar window -> tiny samples, descriptive only, NOT validation.
"""
...
rows=[
 ("Real account (no stop, all hrs)",None),
 ("Maker — 03-09 window",          sim("maker",window=True)),
 ("Maker — all hours",             sim("maker")),
 ("Maker — 03-09, low-tempo",      sim("maker",window=True,lowtempo=True)),
 ("Maker — 03-09, LOW-vol",        sim("maker",window=True,vlo=1.0)),
 ("Maker — 03-09, HIGH-vol",       sim("maker",window=True,vhi=1.3)),
 ("Taker — 03-09 window",          sim("taker",window=True)),
 ("Taker — all hours",             sim("taker")),
]
print(f"=== Over real-account window {P0} -> {P1} (~2 days, descriptive only) ===")
print(f"{'variant':34s} {'trades':>6} {'WR':>6} {'PnL$':>9}")
print(f"{'Real account (no stop, all hrs)':34s} {67:>6} {'92.5%':>6} {'+452.44':>9}")
for name,p in rows[1:]:
    if p is None or len(p)==0:
        print(f"{name:34s} {0:>6} {'--':>6} {'$0':>9}");continue
    print(f"{name:34s} {len(p):>6} {100*(p>0).mean():>5.0f}% {p.sum()*USD:>+9.0f}")
print("\nNOTE: 2-day window, n<=46 -> descriptive comparison, not statistical validation.")
