...
for H in (1,2,3,5,15,30,60,120,240):
...
rel=np.abs(dist)/atr; rel=np.where(np.isnan(rel), np.nanmedian(rel), rel); print("rel gap quantiles", np.quantile(rel,[.25,.5,.75,.9]))
qs=np.quantile(rel,[0,.25,.5,.75,1.0]); qs[-1]+=1e-9; (0)
for i in range(4):
...
# conditional: among events not hit within first 3 bars, hit within 60 (obs vs null conditioned same way)
o3,_=hitm(t,lvl,up,3); 
tt3 = t + pd.Timedelta("3min")
