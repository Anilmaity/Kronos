...
def flips(b):
    ...
    for i in range(n):
        ...
        if last_hi is not None and c[i] > h[last_hi]:
            if prev_break == -1:
                a = last_hi + 1 + int(np.argmin(l[last_hi+1:i+1]))
                out.append((i, 1, a, l[a], h[a:i+1].max()))
            prev_break, last_hi = 1, None
        if last_lo is not None and c[i] < l[last_lo]:
            if prev_break == 1:
                a = last_lo + 1 + int(np.argmax(h[last_lo+1:i+1]))
                out.append((i, -1, a, h[a], l[a:i+1].min()))
            prev_break, last_lo = -1, None
        j = i - 2
        if j >= 2:
...
if __name__ == "__main__":
    ...
    ev.to_pickle(f"indepX_{int(cot)}{int(absl)}.pkl")
