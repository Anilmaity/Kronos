...
import os
TIE=float(os.environ.get('TIE','0')); AMB=[]
...
def resolve(i0,t0,sg,st,tg):
    ...
    for s in range(0,n,20000):
        ...
        k=np.arange(len(ii))
        same=(fs==ft)&(fs<MAXB); ob=O[idx[k,np.minimum(fs,MAXB-1)]]
        gs=np.where(g[:,0]>0,ob<=stp[:,0],ob>=stp[:,0]); gt=np.where(g[:,0]>0,ob>=tgt[:,0],ob<=tgt[:,0])
        amb=same&~gs&~gt
        stopfirst=(fs<ft)|(same&gs)|(amb&(TIE<=0.5)); tgtfirst=(ft<fs)|(same&~gs&gt)|(amb&(TIE>=0.5))
        ...
        px=np.where(amb&(TIE==0.5),0.5*stp[:,0]+0.5*tgt[:,0],px)
        AMB.append(amb)
