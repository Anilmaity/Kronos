exec(open("indep.py").read().split('t0=ev.dt.values')[0])
def run(buf_bp=0.0, buf_abs=0.0, seed=7):
    ...
    for k in range(5):
        ...
        ct=np.clip(np.searchsorted(grid,(t0+off).astype(np.int64)),0,len(grid)-1)
...
for kw in [dict(),dict(seed=11),dict(buf_abs=0.10),dict(buf_abs=0.20),dict(buf_abs=0.30),dict(buf_bp=1),dict(buf_bp=2)]:
    t0,R,cR,risk=run(**kw)
