"""Shared helpers for batch time_own_02b (time category, TTrades own voice).

Declared BEFORE any test in this batch was run:
- cisd_book: the phase-3 locked bare CISD rung-0 book (series_open, swing 2/2, max_wait 3,
  stop at the protected swing, 2R), max hold = 10 entry-TF periods (prereg spec §5.5).
- FOMC scheduled statement dates (14:00 NY), public Fed calendar 2016-2026H1; emergency
  inter-meeting actions (2020-03-03, 2020-03-15) excluded; 2026 limited to dates also carried
  by KronosStrategies/strategies/shared/event_gate.py (Jan 28, Mar 18) -- later 2026 dates
  omitted as uncertain (any omission can only attenuate a gate differential).
- NFP release dates from the BLS Employment Situation scheduling rule (same rule as
  risk_guest_02b/pre-event-avoidance-rule; 2025 shutdown special cases).
"""
import sys
...
PH3 = "phase3: meta/conjunction_preregistration.md §1.8-1.13 (locked rung-0 config); max hold = 10 entry-TF periods (§5.5)"
...
def base_rule(tf):
    return (f"baseline book: phase-3 bare {tf} CISD (series_open level, 2/2 swings, max_wait 3), "
            f"decide at the confirming bar close, enter next M1 open, stop at protected swing, 2R, "
            f"hold {HOLD[tf]}")
...
def ny_date(t):
    """Session date (18:00 NY roll; Sunday reopen -> Monday) as naive date."""
    return pd.DatetimeIndex(cl.session_date(pd.DatetimeIndex(t))).tz_localize(None).normalize()


def summarize(res):
    keys = ("n", "diff", "ci_lo", "ci_hi", "p", "mde", "verdict", "verdict_detail",
            "dependence", "ties", "ctrl_overlap")
    for k in keys:
        print(k, res.get(k))
    print("control", res.get("control"))
    print("gate", {k: v for k, v in res.items() if "gate" in k or "fire" in k})
